...
export const ArtistProfile: React.FC<ArtistProfileProps> = ({
  ...
  const { tracks, setCurrentTrack, setIsPlaying, isPlaying, currentTrack } =
    useMusic();
  ...
  const [comments, setComments] = useState<Array<{
  ...
  }>>([
    {
      id: 1,
      user: "NeonRider",
      text: "This artist literally rewired my brain.",
      likes: 124,
      dislikes: 4,
      time: "2h ago",
      userRating: null,
    },
    {
      id: 2,
      user: "SynthWave_99",
      text: "The production on the latest album is out of this world.",
      likes: 89,
      dislikes: 1,
      time: "5h ago",
      userRating: null,
    },
    {
      id: 3,
      user: "CyberPunk_2077",
      text: "Listening to this on a late night drive is an experience.",
      likes: 56,
      dislikes: 2,
      time: "1d ago",
      userRating: null,
  ...
  const handleLikeComment = (commentId: number) => {
    setComments(comments.map(c => {
      if (c.id === commentId) {
        const currentRating = c.userRating;
        let newLikes = c.likes;
        let newDislikes = c.dislikes;
        let newUserRating: "like" | "dislike" | null = null;
  ...
  const uniqueAlbums = Array.from(
    new Set(artistTracks.map((t) => t.album)),
  ).map((albumName) => {
    const trackForAlbum =
      artistTracks.find((t) => t.album === albumName) || artistTracks[0];
    return {
      name: albumName,
      trackId: trackForAlbum?.id || "1",
      coverUrl:
        trackForAlbum?.coverUrl ||
        `https://picsum.photos/seed/${albumName}/400/400`,
  ...
  const customPlaylists = [
    {
      ...
      coverUrl:
        ...
        `https://picsum.photos/seed/${artistName}p1/400/400`,
  ...
  const fansAlsoLike = [
    {
      name: "Kavinsky",
      coverUrl: `https://ui-avatars.com/api/?name=Kavinsky&background=random&size=256`,
    },
    {
      name: "Daft Punk",
      coverUrl: `https://ui-avatars.com/api/?name=Daft+Punk&background=random&size=256`,
    },
    {
      name: "Justice",
      coverUrl: `https://ui-avatars.com/api/?name=Justice&background=random&size=256`,
    },
  ];
  ...
  return (
    <div
      ref={scrollRef}
      className="relative overflow-y-auto overflow-x-hidden min-h-screen pb-36 scroll-smooth"
    >
      {/* ─── TOAST NOTIFICATION ─────────────────────────────────────────── */}
      <AnimatePresence>
        {toastMessage && (
            ...
            <div className="flex-1 min-w-0 pb-1 md:pb-3">
              ...
              >
                <span className="flex items-center gap-1.5">
                  ...
                  <span className="text-white">{listenerCount}</span>
                  &nbsp;monthly listeners
                </span>
                <span className="w-px h-3 bg-white/10 hidden sm:block" />
                <span className="flex items-center gap-1.5">
                  <Disc size={12} className="text-accent" />
                  <span className="text-white">{uniqueAlbums.length}</span>
                  &nbsp;albums
                </span>
                <span className="w-px h-3 bg-white/10 hidden sm:block" />
                <span className="flex items-center gap-1.5">
                  <Layers size={12} className="text-accent" />
                  <span className="text-white">{artistTracks.length}</span>
                  &nbsp;tracks
                </span>
                <span className="w-px h-3 bg-white/10 hidden sm:block" />
                <span className="flex items-center gap-1.5">
                  ...
                  <span className="text-white ml-1">
                    {artistTracks[0]?.genre || "Electronic"}
                  </span>
                </span>
              </motion.div>
            </div>
      ...
      <div className="relative z-10 w-full">
                ...
                >
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                      ...
                      {isCurrentlyPlaying ? (
                        ...
                        <>
                          ...
                          <Play
                            fill="currentColor"
                            size={12}
                            className="mx-auto hidden group-hover:block ml-2.5"
                          />
                        </>
                      )}
                    </div>
                    ...
                    <div className="flex-1 min-w-0 pr-4">
                      <h3
                        className={`text-sm font-bold truncate ${isCurrentlyPlaying ? "text-accent" : "text-white"}`}
                      >
                        {track.title}
                      </h3>
                    ...
                    <div className="relative">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          setActiveDropdownId(
                            activeDropdownId === track.id ? null : track.id,
                          );
                        }}
            ...
            <div className="relative z-10 w-full md:w-2/5 p-8 md:p-10 flex flex-col justify-between border-b md:border-b-0 md:border-r border-white/8">
              <div>
                <div className="flex items-center gap-2 text-accent text-[10px] font-black tracking-[0.3em] uppercase mb-3">
                  <Sparkles size={12} className="animate-pulse" /> Cortex
                  Intelligence
                </div>
                ...
                <p className="text-sm text-white/50 leading-relaxed">
                  An hour of seamless algorithmically-blended audio — deepest
                  cuts fused with your exact sonic taste.
                </p>
              </div>
              <div className="mt-7 flex items-center gap-4">
                ...
                <div>
                  <div className="text-xs font-black tracking-widest uppercase text-white">
                    {isCortexMixPlaying ? "Playing" : "Play Mix"}
                  </div>
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-10 lg:px-16 pb-16">
          <div className="flex items-center justify-between mb-8">
            ...
            >
              {showAllAlbums ? "Collapse" : "See all"}{" "}
              <ChevronRight
                size={14}
                className={`transition-transform duration-300 ${showAllAlbums ? "rotate-90" : ""}`}
              />
            </button>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {(showAllAlbums ? uniqueAlbums : uniqueAlbums.slice(0, 4)).map(
              (album, idx) => (
                <motion.div
                  key={idx}
                ...
                >
                  ...
                  <h3 className="text-sm font-black tracking-tight truncate group-hover:text-accent transition-colors">
                    {album.name}
                  </h3>
                  <p className="text-[10px] font-bold text-white/35 tracking-widest uppercase mt-1">
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-10 lg:px-16 pb-16 grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div className="flex flex-col">
            ...
            <div className="space-y-3 flex-1">
              {customPlaylists.map((pl, idx) => {
                const isMixPlaying =
                  isPlaying && currentTrack?.album === pl.name;
                return (
                  <motion.div
                    key={pl.id}
          ...
          <div className="flex flex-col">
            ...
            <div
              className={`flex-1 bg-gradient-to-br ${isRadioActive ? "from-accent/20 to-accent/5 border-accent/30" : "from-white/8 to-white/[0.02] border-white/8"} p-7 rounded-3xl border relative overflow-hidden group flex flex-col justify-between min-h-[220px] transition-colors duration-500`}
            >
              <div className="absolute top-0 right-0 p-6 opacity-[0.06] group-hover:opacity-[0.12] group-hover:scale-110 transition-all duration-500 pointer-events-none">
                <Zap
                  ...
                  className={
                    isRadioActive
                      ? "text-accent drop-shadow-[0_0_15px_rgba(255,255,255,0.5)]"
                      : ""
                  }
                />
              </div>
              <div>
                ...
                <p className="text-sm text-white/45 max-w-xs leading-relaxed">
                  Continuous algorithmic mix exploring similar frequencies,
                  influences, and deep cuts.
                </p>
              </div>
              <button
                onClick={toggleRadio}
          ...
          <div className="flex flex-col">
            ...
            <div className="grid grid-cols-3 gap-4 flex-1 content-start">
              {[
                {
                  name: "Kavinsky",
                  img: "https://picsum.photos/seed/kav/400/400",
                  time: "2h ago",
                },
                {
                  name: "Daft Punk",
                  img: "https://picsum.photos/seed/daft/400/400",
                  time: "5h ago",
                },
                {
                  name: "Justice",
                  img: "https://picsum.photos/seed/justice/400/400",
                  time: "1d ago",
                },
              ].map((artist, idx) => (
                <motion.div
                ...
                >
                  ...
                  <h3 className="text-xs font-black tracking-tight truncate w-full group-hover:text-accent transition-colors">
                    {artist.name}
                  </h3>
                </motion.div>
              ))}
          ...
          <div className="flex flex-col">
            ...
            >
              <input
                ...
                className="flex-1 bg-white/[0.03] border border-white/10 hover:border-white/20 focus:border-accent/40 rounded-xl px-4 py-3 text-sm font-medium text-white placeholder-white/30 focus:outline-none focus:bg-white/[0.05] transition-all"
              />
              <button
                type="submit"
                className="px-6 py-3 rounded-xl bg-accent text-background text-xs font-black uppercase tracking-widest hover:bg-accent/90 transition-all duration-300 active:scale-95 shrink-0"
              >
                Post
              </button>
            </form>
                  ...
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button 
                        ...
                        className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-white/[0.02] hover:bg-white/[0.08] transition-all duration-200 border ${
                        ...
                        title="Like comment"
                      >
                        <ThumbsUp size={11} className={comment.userRating === "like" ? "fill-accent/20" : ""} />
                      ...
                      >
                        <ThumbsDown size={11} className={comment.userRating === "dislike" ? "fill-red-400/20" : ""} />
                        <span className="text-[10px] font-mono font-black">
                          {comment.dislikes}
                        </span>
                      </button>
